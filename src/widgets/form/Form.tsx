...
export const Form = () => {
  ...
  const handleFormSubmit = () => {
    ...
    if (notHasError) {
      auth(inputLoginValue, inputPasswordValue, setIsLoading, setHasLoginError, setHasPasswordError, setErrorApiMessage, navigate);
    }
  };
  ...
  return (
    <form className="grid grid-cols-1 gap-4">
      ...
      <div className='relative flex flex-col gap-1.5'>
        ...
        <InputFeedback
          error={hasLoginError}
          errorMessage={errorApiMessage ? errorApiMessage : 'Неверный логин. Проверьте правильность ввода или создайте новый аккаунт'}
        />
        <InputFeedback
          success={(!hasLoginError && hasPasswordError)}
          successMessage="Валидация пройдена"
        />
        <p className='text-light-neutral-600 text-xs'>Имя должно начинаться с заглавной буквы, содержать 2–30 символов, без пробелов</p>
      </div>
      <div className='relative flex flex-col gap-1.5'>
        <Input
          id="password"
        ...
        {inputPasswordValue && (
          <div className='flex absolute top-[40px] right-[12px]'>
            ...
            <button type='button' disabled={isLoading} onClick={(e) => {
              ...
              {showPassword ? <Icon name={IconsEnum.visibilityOff} /> : <Icon name={IconsEnum.visibilityOn} />}
            </button>
          </div>
        )}
        <InputFeedback
        ...
        <InputFeedback
          ...
          successMessage="Валидация пройдена"
        />
        <p className='text-light-neutral-600 text-xs'>Пароль должен содержать 8–30 символов, без пробелов. Минимум 2 цифры и 3 заглавные буквы</p>
      </div>
      <Button
        fullWidth
        onClick={(event) => {
        ...
        isLoading={isLoading}
        className={(inputLoginValue === '' || inputPasswordValue === '') ? 'text-light-neutral-400' : undefined}
        bgType={(inputLoginValue === '' || inputPasswordValue === '') ? 'neutral' : 'primary'}
